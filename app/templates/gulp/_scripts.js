...
<% if (props.jsPreprocessor.srcExtension !== 'es6' &&  props.jsPreprocessor.key !== 'typescript') { -%>
  ...
  function buildScripts() {
    ...
    if (props.jsPreprocessor.key === 'coffee') {
      -%> .pipe($.coffeelint()).pipe($.coffeelint.reporter()).pipe($.coffee()).on('error', conf.errorHandler('CoffeeScript')) <%
    }
    if (props.jsPreprocessor.key !== 'noJsPrepro') {
      -%> .pipe($.sourcemaps.write()).pipe(gulp.dest(path.join(conf.paths.tmp, '/serve/app'))) <%
    } -%> .pipe($.size())
  }; <%
} else {
  -%>
      ...
      module: { <%
        ...
        if (props.jsPreprocessor.key === 'traceur') {
          -%> loaders: [{
            test: /\.js$/,
            exclude: /node_modules/,
            loaders: ['ng-annotate', 'traceur-loader']
          }] <%
        }
        if (props.jsPreprocessor.key === 'typescript') {
          -%> loaders: [{
